...
echo $$ > $DSTAT_HOMEDIR/monitor.pid
...
then
 ...
 if [ ""$DSTAT_MONITOR_MYSQL == "true" ]
  then 
   ...
   $DSTAT_HOMEDIR/dstat --noupdate -T -l -f -c -m -n -d -r --aio -s -g --vm --fs  -i -y -p --disk-util --mysql5-all1 --mysql-ndb --client-events --output $DSTAT_HOMEDIR"/log_exp_"$DSTAT_EXPERIMENT_ID".csv"  $DSTAT_MONITORING_FREQUENCY
