 ...
 */
public class TransactionReaderTwo implements TransactionReader
{
	private DBSeerDataSet dataset;
	private ArrayList<DBSeerTransactionSample> sampleList;
	private int sampleIndex;
	private int type;

	public TransactionReaderTwo(DBSeerDataSet dataset, int type)
	{
	...
	{
		this.sampleList = dataset.getTransactionSampleLists().get(type).getSamples();
	...
	{
		if (sampleList.size() <= sampleIndex)
		{
			return "";
		}
		else
		{
			DBSeerTransactionSample sample = sampleList.get(sampleIndex++);
			return sample.getStatement();
		}
	}
