...
/**
 * Created by dyoon on 10/3/15.
 */
public class LiveSystemLogTailerListener extends TailerListenerAdapter
{
	private File datasetHeaderFile;
	...
	public LiveSystemLogTailerListener(TransactionMap map, PrintWriter monitorWriter, File datasetHeaderFile, boolean newStart)
	{
		this.map = map;
		this.monitorWriter = monitorWriter;
		this.datasetHeaderFile = datasetHeaderFile;

		this.metaHeaders = null;
		this.headers = null;
	...
	public void handle(String s)
	{
		...
		String[] logs = s.split(System.getProperty("line.separator"));

		for (String log : logs)
		{
			// skip first few lines
			if (log.isEmpty() || log.contains("Dstat") || log.contains("Author") || log.contains("Host") || log.contains("Cmdline"))
			{
				continue;
			}

			if (!newStart && log.contains("epoch"))
			{
				continue;
			}
			...
			else
			{
				...
				long time = Math.round(Double.parseDouble(tokens[0]));
				map.addSysLog(time, log);
			}
		}
	}
	...
	private boolean writeHeader()
	{
		...
		File        file             = this.datasetHeaderFile;
