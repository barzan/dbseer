...
function predictions = useLockModelNew(datasetPath, counts)

[tps J f K regions D S0 S g] = prepareLockModelNew(datasetPath, counts(1,:));
...
for row=1:nRows
    [tps J f K regions D S0 S g] = prepareLockModelNew(datasetPath, counts(row,:));
    filter = f ./ f;
