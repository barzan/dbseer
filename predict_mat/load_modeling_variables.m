function [mvGrouped mvUngrouped] = load_modeling_variables(data_dir, signature, groupingStrategy)
overallTime = tic;

mvGrouped = struct();
mvUngrouped = struct();

mvUngrouped.prclat = load([data_dir '/trans-',signature,'_prctile_latencies.mat']);
mvGrouped.prclat = mvUngrouped.prclat;

[monitor avglat IndividualCounts dM] = load3_offset(data_dir, signature, 0, 10, true);
if nargin > 2
